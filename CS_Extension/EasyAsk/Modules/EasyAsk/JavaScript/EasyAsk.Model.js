define('Kodella.EasyAsk.EasyAsk.Model'
    ...
    , function
        ...
        return BackboneCachedModel.extend({
            urlRoot: Utils.getAbsoluteUrl(getExtensionAssetsPath('services/EasyAsk.Service.ss'))
        ,   defaults: {
            ...
            , getResults: function(cat, pageSize, page, order, urlParams)
            {
                ...
                ,   self = this
                ,   requestDataParam = cat.indexOf('?') >= 0 ? encodeURIComponent(page) : 'CA_BreadcrumbClick'
                ,   requestActionParam = cat.indexOf('?') >= 0 ? 'navbar' : 'advisor'
                ,   rootProds = cat == 'All Products' ? '&rootprods=1' : ''
                ,   newURL = this.formURL(pageSize, order) + '&RequestAction=' + requestActionParam + '&RequestData='+requestDataParam+'&CatPath=' + encodeURIComponent(cat) + curpage + rootProds;
                ...
                /*if(urlParams != "")
                {
                    console.log("here if model");
                    if(pageSize && page == 'first' && order)
                    {
                        console.log("here if if model");

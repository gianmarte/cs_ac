define('Kodella.EasyAsk.Facets.Browse.View.Ext'
	, [
		...
		//, 'Kodella.EasyAsk.Facets.ItemListDisplaySelector.View.Ext'
	...
	, function (
		...
		, GlobalViewsPaginationExt
		//, FacetsItemListDisplaySelectorViewExt
		...
		return FacetsBrowseView.extend({
			...
			, initialize: function (options) {
				...
				this.itemsDisplayOptions = Utils.deepCopy(options.application.getConfig('easyAsk.itemsDisplayOptions'));
			...
			, _gotoFacets: function _gotoFacet(e) 
			{
				var curURL = new URL(window.location.href)
					...
					hasKeyword ? newURL.searchParams.set('keywords', hasKeyword) : "";
					hasPageSize ? newURL.searchParams.set('show', hasPageSize) : "";
					hasSort ? newURL.searchParams.set('order', hasSort) : "";
					

					window.location.hash != "" 
						? curURL.hash = hasKeyword ? searchParam + newURL.search : ea_seopath + newURL.search
						: curURL.pathname = hasKeyword ? searchParam : ea_seopath;

					
					var final_href = window.location.hash != "" ? decodeURIComponent(curURL.hash) : curURL.pathname + decodeURIComponent(newURL.search).replace("+", " ");
			...
			, childViews: _.extend(FacetsBrowseView.prototype.childViews,
				{
					...
					, 'Facets.Items': function () {
						...
						console.log("display_option", display_option);

						console.log("display_option.template", display_option.template);
					...
					, 'GlobalViews.Pagination': function () {
						...
						return new GlobalViewsPaginationExt(_.extend({
							currentPage: self.products.currentPage
					...
					/*,	'Facets.ItemListDisplaySelector': function()
						{
							return new FacetsItemListDisplaySelectorViewExt({
